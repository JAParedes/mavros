...
int main(int argc, char* argv[]) {
    ...
    string serverAddress = "192.168.254.1"; //"tracking1";		// The address of the computer connected to the Qualisys motion tracking system (ex: "130.75.144.179")
                    ...
                    if (bodyCount <= 0) {
                        ...
                        for (int i = 0; i < bodyCount; i++) {
                            ...
                            if (isnan(x)||isnan(y)||isnan(z)||isnan(roll)||isnan(pitch)||isnan(yaw)) {
								...
								pose.pose.position.x = y/1000.0f;
								pose.pose.position.y = x/1000.0f;
								pose.pose.position.z = -z/1000.0f;
								
								tf::Quaternion qtemp = tf::createQuaternionFromRPY(roll*deg2rad, pitch*deg2rad, yaw*deg2rad);
								pose.pose.orientation.x = qtemp.y();
								pose.pose.orientation.y = qtemp.x();
