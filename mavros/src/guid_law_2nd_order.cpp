...
class WP_Filter {
	...
	private:
		...
		bool max_vel_en;
	public:
		WP_Filter(float init_val, float zeta, float wn, float sp_pub_rate, float maxVelMag){
			yk1 = init_val;
			yk2 = init_val;
...
void callbackPostSetpoints(const ros::TimerEvent& event)
{
	...
	static WP_Filter filt_x(0.0f,zeta_filt, wn_filt, sp_rate, max_vel_xyz_filt, max_vel_on);
	static WP_Filter filt_y(0.0f,zeta_filt, wn_filt, sp_rate, max_vel_xyz_filt, max_vel_on);
	static WP_Filter filt_z(0.0f,zeta_filt, wn_filt, sp_rate, max_vel_xyz_filt, max_vel_on);
	static WP_Filter filt_yaw(0.0f,zeta_filt, wn_filt, sp_rate, max_vel_yaw_filt, max_vel_on);
...
int main(int argc, char **argv)
{
	...
	bool ok6 = ros::param::get("~max_vel_on", max_vel_on) ;
	if (!ok6){max_on = true;}
	
	bool ok7 = ros::param::get("~max_vel_xyz_filt", max_vel_xyz_filt) ;
	if (!ok7){max_vel_xyz_filt = 2.0f;}
	
	bool ok8 = ros::param::get("~max_vel_yaw_filt", max_vel_yaw_filt) ;
	if (!ok8){max_vel_yaw_filt = 30.0f;}

	bool ok9 = ros::param::get("~land_end", land_end) ;
	if (!ok9){land_end = true;}
