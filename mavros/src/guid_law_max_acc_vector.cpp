...
class WP_Filter_Position {

	private:
		float dT;
		float t;
		float x0_x, x0_y, x0_z;
		float xr_x, xr_y, xr_z;
		float d_xr_abs;
		float d_xT1_abs;
		...
		bool trapzVel;
	public:
		WP_Filter_Position(float sp_pub_rate, float x0_x_par, float x0_y_par, float x0_z_par, 
		float xr_x_init, float xr_y_init, float xr_z_init, float v_max_par, float a_max_par){
			dT = sp_pub_rate;
			t = 0.0f;
			x0_x =  x0_x_par; x0_y =  x0_y_par; x0_z =  x0_z_par;
			xr_x = xr_x_init; xr_y = xr_y_init; xr_z = xr_z_init;
			float d_xr_x = xr_x - x0_x; float d_xr_y = xr_y - x0_y; float d_xr_z = xr_z - x0_z;
			d_xr_abs = std::sqrt( (d_xr_x)*(d_xr_x) + (d_xr_y)*(d_xr_y) + (d_xr_z)*(d_xr_z) );
			unit_vec_x = d_xr_x / std::min(d_xr_abs, 0.001f);
			unit_vec_y = d_xr_y / std::min(d_xr_abs, 0.001f);
			unit_vec_z = d_xr_z / std::min(d_xr_abs, 0.001f);
			v_max = v_max_par;
			a_max = a_max_par;
			T1 = v_max / a_max;
			T2 = T1 + d_xr_abs/v_max - v_max/a_max;
			T3 = T1 + T2;
			d_xT1_abs = std::fabs(0.5*a_max*T1*T1);
			v_max_bar = std::sqrt(d_xr_abs*a_max);
			T1_bar = std::sqrt(d_xr_abs/a_max);
			trapzVel = (d_xr_abs > 2.0f*d_xT1_abs);
		}
		void newSP(float xr_x_n, float xr_y_n, float xr_z_n){
			x0_x =  xr_x; x0_y =  xr_y; x0_z =  xr_z;
			t = 0.0f;
			xr_x = xr_x_n; xr_y = xr_y_n; xr_z = xr_z_n;
			float d_xr_x = xr_x - x0_x; float d_xr_y = xr_y - x0_y; float d_xr_z = xr_z - x0_z;
			d_xr_abs = std::sqrt( (d_xr_x)*(d_xr_x) + (d_xr_y)*(d_xr_y) + (d_xr_z)*(d_xr_z) );
			unit_vec_x = ((d_xr_abs > 0.01) ? (d_xr_x / d_xr_abs) : 0.0f);
			unit_vec_y = ((d_xr_abs > 0.01) ? (d_xr_y / d_xr_abs) : 0.0f);
			unit_vec_z = ((d_xr_abs > 0.01) ? (d_xr_z / d_xr_abs) : 0.0f);
			T2 = T1 + d_xr_abs/v_max - v_max/a_max;
			T3 = T1 + T2;
			v_max_bar = std::sqrt(d_xr_abs*a_max);
			T1_bar = std::sqrt(d_xr_abs/a_max);
		...
		void run_filt(float xr_x_comm, float xr_y_comm, float xr_z_comm, float& x_sp, float& y_sp, float& z_sp){
			if (( (xr_x_comm - xr_x)*(xr_x_comm - xr_x) + 
			(xr_y_comm - xr_y)*(xr_y_comm - xr_y) + 
			(xr_z_comm - xr_z)*(xr_z_comm - xr_z) ) > 0.001f)
			{
				this->newSP(xr_x_comm, xr_y_comm, xr_z_comm);
			}
			t += dT;
			float rk = 0.0f;
			if (trapzVel)
			{
				...
				{
					rk = (0.5*a_max*T1*T1 + v_max*(t - T1));
				}
				else if ((T2 <= t) && (t < T3))
				{
					rk = (0.5*a_max*T1*T1 + v_max*(T2 - T1)
					+ v_max*(t - T2) - 0.5*a_max*(t - T2)*(t - T2));
				}
				else
				{
				...
				{
					rk = (0.5*a_max*T1_bar*T1_bar + v_max_bar*(t - T1_bar)
					 - 0.5*a_max*(t - T1_bar)*(t - T1_bar));
				}
				else
				{
			...
			x_sp = x0_x + rk*unit_vec_x;
			y_sp = x0_y + rk*unit_vec_y;
			z_sp = x0_z + rk*unit_vec_z;
		}
};
...
void callbackPostSetpoints(const ros::TimerEvent& event)
{
	...
	static WP_Filter_Position filt_xyz(sp_rate, spp2_x, spp2_y, spp2_z, 
		spp2_x, spp2_y, spp2_z, vel_xyz_max, acc_xyz_max);
	...
	float spp3_x, spp3_y, spp3_z;
	filt_xyz.run_filt(spp2_x, spp2_y, spp2_z, spp3_x, spp3_y, spp3_z);
	pose.pose.position.x =  spp3_y;
	pose.pose.position.y = spp3_x;
	pose.pose.position.z = -spp3_z;
...
int main(int argc, char **argv)
{
	...
	std::string wp_str;
	bool ok10 = ros::param::get("~wp_str", wp_str) ;
	if (!ok10)
	{
